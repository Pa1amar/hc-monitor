# Install and uninstall. install asks, in order: [install curl?] -> URL -> services -> "Change thresholds?".

test_install_fresh() {
    rm "$ROOT_DIR/etc/hc-monitor.conf"
    printf 'nginx active\n' > "$STUB_DIR/services"
    INPUT=$'https://hc-ping.com/new-check\nnginx\n\n'   # URL, services: nginx, thresholds: Enter (keep)
    run_script install
    assert_rc 0
...
test_reinstall_keeps_current_values() {
    write_conf 'HC_PING_URL="http://127.0.0.1:1/keep"' 'SERVICES="cron"' \
